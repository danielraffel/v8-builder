...
| P1.1 | Linux x64 sealed shared .so | WIP | GitHub x64 building (run 26942225154). Iter1: use_custom_libcxx=false→old sysroot libstdc++→C++20 fail (bit_cast/source_location). FIXED (D2b): Linux uses V8 bundled libc++ (sealed, no shared C++ types w/ Skia). Now past compile, doing full V8 build (~30-60m); next checks = gn ELF seal target + validator. |
