...
| P1d | Validate sealed-from-source V8 ⟷ Dawn (no link flag) | WIP | Demo built+links OUR @rpath/libv8.dylib (not libnode); choc compiles clean vs V8 15.1. **Abseil collision GONE (no PerTableSeed)** — seal works! Then hit a clean V8 fatal-check: pointer-compression mismatch (provider ON vs embedder OFF, D3). Rebuilding+resealing with v8_enable_pointer_compression=false for libnode parity. |
