...
| P1.1 | Linux x64 sealed shared .so | WIP(slow-infra) | libc++ blocker FIXED (D2b: Linux uses V8 bundled libc++). Build PROGRESSES cleanly but GitHub free ubuntu runner takes >1h on the V8 compile (no ccache/few cores) — hasn't reached seal/validate. Free CI is too slow to iterate. Unblock: faster runner (paid/large GitHub, or Tart arm64 once networked) OR just more wall-clock. Next checkpoints: ELF version-script seal + validator. |
...
| P2.1 | macOS shared lib (arm64, x86_64, universal), sealed | WIP | arm64 DONE (P1d). x86_64 cross-build in flight locally (gn gen OK, 1356 targets); then lipo→universal + validate x86_64 under Rosetta. Uses available mac hardware while Linux CI grinds. |
