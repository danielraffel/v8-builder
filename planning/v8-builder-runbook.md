...
| P1.1 | Linux x64 sealed shared .so | WIP | clean x64 CI run in flight (GitHub ubuntu-24.04). build-v8.py linux lane + gn seal target + standalone validator wired. Local Tart is arm64-only (great for linux/arm64 + fast recipe iteration, not x64). Expect ABI/seal iteration. |
