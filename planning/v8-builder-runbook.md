...
| P1.1 | Linux x64 sealed shared .so | BLOCKED(seal bug + slow infra) | V8 COMPILES on Linux (libc++ fix worked). **Seal FAILS: ld.lld duplicate-symbol** — ELF gn target links monolith twice (deps=[:v8_monolith] AND --whole-archive); lld doesn't dedup under --whole-archive (mac's -force_load did). FIX: monolith must appear ONCE (whole-archive XOR deps) while still pulling the Rust closure — needs gn iteration. BLOCKER: each verify = ~2h GitHub build; needs faster x64 Linux runner to iterate. macOS matrix is the verified analog. |
