...
| Linux x64 (RHEL/Rocky-8 floor) | 🔧 lane added | ⚪ | ⚪ | **WIP (task #24b)** — old-glibc-floor release portability. Same `linux_gn_args` (`use_sysroot=false`+`use_custom_libcxx=false`), but built INSIDE a Rocky Linux 8 container (glibc 2.28) so the sealed `libv8.so` loads on any glibc ≥ 2.28 host (the ubuntu-24.04 build floors at ~2.39 and won't dlopen on RHEL/Rocky 8). gcc-toolset-12 supplies C++20 libstdc++ on the 2.28 base. Acceptance gate = `objdump -T` floor ≤ 2.28 (`tools/rhel8/check_glibc_floor.py`) + unchanged seal audit + identity. Lane: `.github/workflows/build-v8-rhel8.yml` (native x86_64 ubuntu runner → rocky:8 container) + `tools/rhel8/{Dockerfile,build-in-docker.sh}`. Branch `feature/rhel8-glibc-floor`. **Not yet run green — needs a CI dispatch to confirm the measured floor.** |
