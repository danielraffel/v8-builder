...
| P1.1 | Linux x64 sealed shared .so | SEAL MECHANISM PROVEN; faithful build owner-gated to CI | **2026-06-04, on real arm64 Linux (QEMU+HVF loop):** the ELF dup-symbol bug is purely link-line ORDER — synthetic ld.lld 18.1.3 repro: plain-deps-archive-before-`--whole-archive` duplicates, but `--whole-archive` BEFORE the plain ref links rc=0 with all members. The CURRENT `v8_sealed_shared` solink line already emits `${ldflags}`(--whole-archive monolith) → `@rsp`(deps) → `${rlibs}` = whole-archive FIRST = the clean order → **no gn change needed; recipe correct as written**. Earlier "dup-symbol" failure was a prior target shape. Faithful end-to-end linux/x64 (bundled clang-23 + Temporal/Rust + bundled libc++) can't build locally (no arm64 Chromium clang/rust; qemu-user segfaults on bundled x86_64 clang/rustc; system clang-18 has clang-23 flag skew) → single owner-gated GitHub ubuntu-24.04 (x86_64) confirmation run. See seal/coff_research.md "ELF seal — RESOLVED". |
