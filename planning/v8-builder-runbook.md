...
| P1.1 | Linux x64 sealed shared .so | BLOCKED(infra) | Code READY (build-v8.py linux + gn ELF seal + validator). 2026-06-04 session: GitHub x64 runner stuck pending (never started); local `pulp-linux-build` Tart VM gets NO IP (no net flags in pulp tooling) + arm64-only; macstudio arm64-only. Unblock = GitHub queue clears OR a working x64 Linux runner. Then build→seal→validate→iterate. |
